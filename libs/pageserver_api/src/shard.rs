...
impl ShardIdentity {
    ...
    pub fn key_range_local_size(&self, range: &Range<Key>) -> u32 {
        let size = self.key_range_local_size_impl(range);
        if cfg!(debug_assertions) {
            // The local size of a range should always be <= the literal number of blocks in the range
            let naive_size = key_range_size(range);
            if size > naive_size {
                panic!(
                    "Calculated local key range size {size} is greater than naive size {naive_size}"
                )
            }
        }
        size
...
mod tests {
    ...
    #[test]
    fn shard_identity_keyspaces_relation_gap() {
        let shard_identity =
