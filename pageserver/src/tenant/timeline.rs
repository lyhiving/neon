...
impl Timeline {
    ...
    pub(crate) async fn detach_from_ancestor(
        ...
        let Some((ancestor, ancestor_lsn)) = self.ancestor_branchpoint.snapshot() else {
        ...
        if ancestor.ancestor_branchpoint.is_some() {
        ...
        assert!(self.ancestor_branchpoint.detach(&ancestor));
        ...
        let reparented = tenant
            ...
            .filter_map(|tl| {
                ...
                tl.ancestor_branchpoint.and_then(|(tl_ancestor, _)| {
                    if Arc::ptr_eq(&tl_ancestor, &ancestor) {
                        Some(tl.clone())
                    } else {
                        None
                    }
        ...
        for timeline in &reparented {
            if let Err(err) = timeline.ancestor_branchpoint.reparent(&ancestor, self) {
                tracing::error!(
                    ?err,
                    timeline_id=%timeline.timeline_id,
                    "TODO: failed to reparent a timeline; would need to block ancestor for further timeline detaches to work against this"
                );
                continue;
            }
...
impl Timeline {
    ...
    async fn gc_timeline(
        ...
        // FIXME: if there is an ongoing timeline_detach, we should just skip gc
